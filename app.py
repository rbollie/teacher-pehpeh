...
try:
    ...
    _CURRICULA = load_all_curricula()
    MOE_AVAILABLE = bool(_CURRICULA)
except Exception:
    _CURRICULA = {}
    MOE_AVAILABLE = False
...
def main():
    ...
    with st.sidebar:
        ...
        _grade_num = int(''.join(filter(str.isdigit, _grade_en.split()[0]))) if any(c.isdigit() for c in _grade_en) else 10
        _size_val=_sizes()[clsz]
        _res_val=_resources()[res]
        _abl_val=_ability()[abl]
        # === MOE CURRICULUM TOGGLE ===
        if MOE_AVAILABLE:
            st.markdown("---")
            st.markdown(f'<div style="color:#F5D998;font-weight:700;font-size:.95rem">🇱🇷 MOE Curriculum</div>', unsafe_allow_html=True)
            moe_on = st.toggle("Align to MOE Curriculum", value=False, key="moe_toggle",
                               help="When ON, topics and AI prompts align to official Liberia Ministry of Education syllabus")
            if moe_on:
                _moe_subjects = get_available_subjects(_CURRICULA)
                st.markdown(f'<div style="background:#2a1a0a;border:1px solid #D4A843;border-radius:8px;padding:8px 12px;font-size:.78rem;color:#F5D998">📘 Loaded: {", ".join(_moe_subjects)}</div>', unsafe_allow_html=True)
        else:
            moe_on = False
    ...
    if conn:
        ...
        # MOE section
        if moe_on:
            moe_html=f'<span style="color:#F5D998">🇱🇷 MOE Aligned</span>'
    ...
    if t1:
     with t1:
        ...
        # === TOPIC SELECTION (MOE-aware) ===
        _moe_active = False  # tracks if this specific subject/grade has MOE data
        _curriculum_details = None
        _curriculum_ctx = ""
        if moe_on and _CURRICULA:
            moe_topics = get_grade_topics(_CURRICULA, _subj_en, _grade_num)
            if moe_topics:
                _moe_active = True
                st.markdown(f'<div style="background:rgba(212,168,67,.12);border:1px solid {C_GOLD};border-radius:8px;padding:6px 12px;margin:4px 0;font-size:.82rem"><span style="color:{C_GOLD};font-weight:700">✅ MOE Curriculum-Aligned Topics</span> — {_subj_en}, Grade {_grade_num}</div>', unsafe_allow_html=True)
                topic = st.selectbox(T("topic"), moe_topics, key="moe_topic")
                _topic_en = topic  # MOE topics are already in English
                # Get curriculum details for this topic
                _curriculum_details = get_topic_details(_CURRICULA, _subj_en, _grade_num, topic)
                _curriculum_ctx = build_curriculum_context(_CURRICULA, _subj_en, _grade_num, topic)
                # Show curriculum preview
                if _curriculum_details:
                    with st.expander(f"📋 View MOE Requirements: {topic}"):
                        objs = _curriculum_details.get("specific_objectives", [])
                        if objs:
                            st.markdown("**Learning Objectives:**")
                            for o in objs: st.markdown(f"- {o}")
                        content = _curriculum_details.get("content_outline", [])
                        if content:
                            st.markdown("**Content Outline:**")
                            for c in content: st.markdown(f"- {c}")
                        acts = _curriculum_details.get("suggested_activities", [])
                        if acts:
                            st.markdown("**Suggested Activities:**")
                            for a in acts: st.markdown(f"- {a}")
                        ln = _curriculum_details.get("local_contextualization_notes", "")
                        if ln: st.markdown(f"**🇱🇷 Liberian Context:** {ln}")
                        mats = _curriculum_details.get("lab_materials", [])
                        if mats: st.markdown(f"**Lab Materials:** {', '.join(mats)}")
            else:
                st.markdown(f'<div style="font-size:.78rem;color:#FFB74D;margin:4px 0">⚡ No MOE data for {_subj_en} Grade {_grade_num} yet — using general topics</div>', unsafe_allow_html=True)
...
Book context: {lit_info.get('genre','')} from {lit_info.get('origin','')}. Themes: {lit_info.get('themes','')}. {lit_info.get('wassce','')}."""
            ...
            # === MOE CURRICULUM INJECTION ===
            if _moe_active and _curriculum_ctx:
                sp += f"\n\nCRITICAL — MINISTRY OF EDUCATION CURRICULUM DATA:\n{_curriculum_ctx}\nYou MUST align your response with these objectives, content requirements, and suggested activities."
            if _moe_active and _curriculum_details:
                objs = _curriculum_details.get("specific_objectives", [])
                if objs:
                    q += "\n\nMOE Learning Objectives (MUST address):\n" + "\n".join(f"  {i+1}. {o}" for i,o in enumerate(objs))
                ln = _curriculum_details.get("local_contextualization_notes", "")
                if ln: q += f"\n\nLiberian contextualization: {ln}"
                mats = _curriculum_details.get("lab_materials", [])
                if mats: q += f"\nAvailable materials: {', '.join(mats)}"
            if exs: q+="\n"+"; ".join(exs)
            ...
            st.session_state.gen_result={"result":result,"task":task,"topic":topic,"img":img,"img_src":img_src,"rs":rs,"grade":grade,"subject":subject,"lit_book":lit_book,"moe_aligned":_moe_active}
        ...
        if gr:
            ...
            st.markdown(f'<div class="rh"><h3>{ico(20)} {gr["task"]} — {gr["topic"]}{lit_tag}</h3><p style="font-size:.8rem;color:#F0D5D5;margin:4px 0 0">{gr.get("grade","")} · {gr.get("subject","")}{moe_tag}</p></div>',unsafe_allow_html=True)
    ...
    if t3:
     with t3:
        # Build MOE supplement for chat if toggle is on
        _chat_moe_supplement = ""
        if moe_on and _CURRICULA:
            _moe_topics_chat = get_grade_topics(_CURRICULA, _subj_en, _grade_num)
            if _moe_topics_chat:
                _chat_moe_supplement = f"\n\nMOE CURRICULUM CONTEXT: You have access to the Liberia Ministry of Education curriculum for {_subj_en} Grade {_grade_num}. Topics covered: {', '.join(_moe_topics_chat)}. When the teacher asks about any of these topics, align your response to MOE objectives and suggest Liberian-contextualized examples."
        _chat_moe_tag = f" · 🇱🇷 MOE" if _chat_moe_supplement else ""
        st.markdown(f'<div style="background:rgba(139,26,26,.12);border:1px solid rgba(178,34,52,.3);border-radius:12px;padding:14px 18px;margin-bottom:10px">{ico(20)} <strong style="color:{C_GOLD}">{T("ask_tp")}</strong> <span style="color:#C0A070;font-size:.85rem">— {grade} · {subject}{_chat_moe_tag}</span></div>',unsafe_allow_html=True)
        ec=st.columns(3)
            ...
            with ec[i]:
                if st.button(f"💡 {ex[:38]}...",key=f"ex{i}",use_container_width=True):
                    ...
                    with st.status(T("thinking"),expanded=True) as status:
                        ...
                        r,m,allr=best_all(build_chat(_region_val,country,_grade_en,_subj_en,_size_val,_res_val,LANGS[lang],_abl_val,school_name)+_chat_moe_supplement,ex,[{"role":x["role"],"content":x["content"]} for x in st.session_state.chat_messages[:-1]])
        ...
        if uq:
            ...
            with st.status(T("thinking"),expanded=True) as status:
                ...
                r,m,allr=best_all(build_chat(_region_val,country,_grade_en,_subj_en,_size_val,_res_val,LANGS[lang],_abl_val,school_name)+_chat_moe_supplement,uq,[{"role":x["role"],"content":x["content"]} for x in st.session_state.chat_messages[-11:-1]])
