...
UI_TEXT={
 "en":{
  ...
  "no_voice_key":"🔇 No voice key found",
  "lbl_setting":"📍 Setting","lbl_grade":"🎓 Grade","lbl_subject":"📚 Subject",
  "lbl_class_size":"👥 Class Size","lbl_resources":"🧰 Resources","lbl_student_level":"📊 Student Level",
  "lbl_task":"📝 Task","lbl_time":"⏱️ Time","lbl_topic":"📖 Topic",
  "select_agent":"Select AI Agent:","select_version":"Which version to send:",
  "suggested_activities":"Suggested Activities:",
  "options_for":"Options for","add_extras":"add extras",
  "configure_classroom":"🌶️ Configure Your Classroom",
  "net_strong":"Strong (WiFi/5G)","net_moderate":"Moderate (4G/3G)",
  "net_slow":"Slow (2G/Edge)","net_very_slow":"Very Slow","net_none":"No Internet",
  "free_account":"Free account — AI generation not active",
  "all_agents":"All","best_answer":"Best answer — all","agents_lbl":"agents",
  "heads_up":"Heads up","heads_up_msg":"Using all {n} agents gives richer results — though it may take a little longer and could use more mobile data on slower connections. Just something to keep in mind!",
  "btn_planning":"📋 Planning","btn_assessment":"📝 Assessment","btn_activities":"🎯 Activities","btn_study":"📚 Study Support",
 ...
 "fr":{
  ...
  "no_voice_key":"🔇 Clé vocale introuvable",
  "lbl_setting":"📍 Contexte","lbl_grade":"🎓 Classe","lbl_subject":"📚 Matière",
  "lbl_class_size":"👥 Taille de classe","lbl_resources":"🧰 Ressources","lbl_student_level":"📊 Niveau des élèves",
  "lbl_task":"📝 Tâche","lbl_time":"⏱️ Durée","lbl_topic":"📖 Sujet",
  "select_agent":"Sélectionner l'agent IA :","select_version":"Quelle version envoyer :",
  "suggested_activities":"Activités suggérées :",
  "options_for":"Options pour","add_extras":"ajouter des options",
  "configure_classroom":"🌶️ Configurer votre classe",
  "net_strong":"Fort (WiFi/5G)","net_moderate":"Modéré (4G/3G)",
  "net_slow":"Lent (2G/Edge)","net_very_slow":"Très lent","net_none":"Pas d'Internet",
  "free_account":"Compte gratuit — Génération IA non activée",
  "all_agents":"Tous","best_answer":"Meilleure réponse — les","agents_lbl":"agents",
  "heads_up":"À noter","heads_up_msg":"Utiliser les {n} agents donne des résultats plus riches — cela peut prendre un peu plus de temps et utiliser plus de données mobiles sur les connexions lentes. Juste pour information !",
  "btn_planning":"📋 Planification","btn_assessment":"📝 Évaluation","btn_activities":"🎯 Activités","btn_study":"📚 Soutien scolaire",
 ...
 "sw":{
  ...
  "no_voice_key":"🔇 Hakuna ufunguo wa sauti",
  "lbl_setting":"📍 Mazingira","lbl_grade":"🎓 Darasa","lbl_subject":"📚 Somo",
  "lbl_class_size":"👥 Ukubwa wa darasa","lbl_resources":"🧰 Rasilimali","lbl_student_level":"📊 Kiwango cha wanafunzi",
  "lbl_task":"📝 Kazi","lbl_time":"⏱️ Muda","lbl_topic":"📖 Mada",
  "select_agent":"Chagua Wakala wa AI:","select_version":"Toleo gani la kutuma:",
  "suggested_activities":"Shughuli Zilizopendekezwa:",
  "options_for":"Chaguzi za","add_extras":"ongeza ziada",
  "configure_classroom":"🌶️ Sanidi Darasa Lako",
  "net_strong":"Nzuri (WiFi/5G)","net_moderate":"Wastani (4G/3G)",
  "net_slow":"Polepole (2G/Edge)","net_very_slow":"Polepole sana","net_none":"Hakuna Intaneti",
  "free_account":"Akaunti ya bure — Utengenezaji wa AI haujawashwa",
  "all_agents":"Wote","best_answer":"Jibu bora — zote","agents_lbl":"mawakala",
  "heads_up":"Kumbuka","heads_up_msg":"Kutumia mawakala {n} wote hutoa matokeo bora zaidi — ingawa inaweza kuchukua muda mrefu kidogo na kutumia data zaidi kwenye miunganisho ya polepole. Kitu cha kukumbuka!",
  "btn_planning":"📋 Mipango","btn_assessment":"📝 Tathmini","btn_activities":"🎯 Shughuli","btn_study":"📚 Msaada wa Masomo",
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander(T("configure_classroom"), expanded=False):
            ...
            region=st.selectbox(T("setting"),list(_regions().keys()),label_visibility="collapsed",format_func=lambda x: f"{T('lbl_setting')}: {x}")
            grade=st.selectbox(T("grade"),_grades(),index=1,label_visibility="collapsed",format_func=lambda x: f"{T('lbl_grade')}: {x}")
            subject=st.selectbox(T("subject"),_subjects(),label_visibility="collapsed",format_func=lambda x: f"{T('lbl_subject')}: {x}")
            clsz=st.selectbox(T("class_size"),list(_sizes().keys()),index=2,label_visibility="collapsed",format_func=lambda x: f"{T('lbl_class_size')}: {x}")
            res=st.selectbox(T("resources"),list(_resources().keys()),index=1,label_visibility="collapsed",format_func=lambda x: f"{T('lbl_resources')}: {x}")
            abl=st.selectbox(T("student_level"),list(_ability().keys()),label_visibility="collapsed",format_func=lambda x: f"{T('lbl_student_level')}: {x}")
    ...
    if conn:
        ...
        # Network section
        _net_label_map={"Strong (WiFi/5G)":T("net_strong"),"Moderate (4G/3G)":T("net_moderate"),"Slow (2G/Edge)":T("net_slow"),"Very Slow":T("net_very_slow"),"No Internet":T("net_none")}
        _conn_label=_net_label_map.get(conn.get("label",""),conn.get("label",""))
        if conn["quality"]=="none":
            net_html=f'<span style="color:#EF5350">🔴 <strong>{T("offline")}</strong></span>'
        elif conn["quality"] in ("high","medium"):
            net_html=f'<span style="color:#81C784">{conn["emoji"]} <strong>{_conn_label}</strong> ({conn["latency_ms"]}ms)</span>'
        else:
            net_html=f'<span style="color:#FFB74D">{conn["emoji"]} <strong>{_conn_label}</strong> ({conn["latency_ms"]}ms)</span>'
        # Models section
        if act:
        ...
        if ELEVENLABS_API_KEY:
            ...
            voice_html=f'<span style="color:#EF9A9A">{T("no_voice_key")}</span>'
            bar_parts.append(voice_html)
    ...
    if t1:
     with t1:
        c1,c2=st.columns(2)
        with c1: task=st.selectbox(T("task"),list(_tasks().keys()),label_visibility="collapsed",format_func=lambda x: f"{T('lbl_task')}: {x}")
        ...
        with c2:
            if _show_time:
                tm=st.selectbox(T("time"),_times(),label_visibility="collapsed",format_func=lambda x: f"{T('lbl_time')}: {x}")
            else:
                tm="N/A"
                st.markdown(f'<div style="background:var(--bg-card);border:1px solid var(--border-color);border-radius:8px;padding:10px 14px;color:var(--text-muted);font-size:.85rem;opacity:.5">{T("lbl_time")}: {T("na")}</div>',unsafe_allow_html=True)
        ...
        if _IS_PARENT_LETTER:
            ...
            topic=st.selectbox(T("topic"),_get_topics(_subj_en),label_visibility="collapsed",format_func=lambda x: f"{T('lbl_topic')}: {x}")
        ...
        if _avail_agents:
            _agent_opts=[f"🌶️ {T('all_agents')} {len(_avail_agents)} {T('agents_lbl')}"]+[f"🌶️ {a}" for a in _avail_agents]
            _agent_sel=st.selectbox(T("select_agent"),_agent_opts,key="agent_pick",label_visibility="collapsed",format_func=lambda x:x)
            if not any(_agent_sel.endswith(a) for a in _avail_agents):
...
Book context: {lit_info.get('genre','')} from {lit_info.get('origin','')}. Themes: {lit_info.get('themes','')}. {lit_info.get('wassce','')}."""
        ...
        if gr:
            ...
            if gr.get("is_parent_letter") and gr.get("pl_delivery"):
                ...
                if len(_send_opts)>1:
                    _send_pick=st.selectbox(T("select_version"),list(_send_opts.keys()),key="pl_pick_model")
                else:
