...
def generate_student_card(student, school_name="", grade="", subject="", country="Liberia"):
    ...
    try:
        ...
        # QR Code — encodes a vCard so any phone scanner shows a contact card
        if QR:
            _risk_pct = f"{len(rsk)*25}%" if rsk else "Low"
            _vcard = "\n".join([
                "BEGIN:VCARD",
                "VERSION:3.0",
                f"FN:{student['name']}",
                f"ORG:{_school}",
                f"TITLE:Student · {grade}",
                f"NOTE:Subject: {subject} | Risk: {risk_label} | Siblings: {student.get('sib','—')} | Mom Edu: {student.get('mom','—')} | Works: {student.get('wk','—')} | Computer: {student.get('cp','—')} | Generated by Teacher Pehpeh – IBT",
                f"URL:https://teacherpehpeh.org",
                "END:VCARD"
            ])
