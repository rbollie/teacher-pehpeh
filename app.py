...
def email_result(content, subject, key_suffix, container=None):
    """Render email controls: mailto link + copy button"""
    ctx = container or st
    with ctx.expander("📧 Email this result", expanded=False):
        ...
        # Footer
        footer = "\n\n---\nGenerated by Teacher Pehpeh by IBT\nhttps://teacher-pehpeh.streamlit.app\nInstitute of Basic Technology | www.institutebasictechnology.org"
        full_body = clean + footer
        # Mailto has ~2000 char URL limit, truncate body if needed
        max_body = 1800
        if len(full_body) > max_body:
            truncated_body = full_body[:max_body] + "\n\n[Content truncated — full version attached or copied below]"
        else:
            truncated_body = full_body
        encoded_subject = urlquote(subject)
        encoded_body = urlquote(truncated_body)
        ...
        with col_a:
            st.markdown(f'<a href="{mailto_url}" target="_blank" style="display:inline-block;background:#2B7DE9;color:white;padding:8px 20px;border-radius:8px;text-decoration:none;font-weight:600;font-size:.9rem;text-align:center;width:100%">📧 Open in Email App</a>', unsafe_allow_html=True)
        with col_b:
            st.download_button("📎 Save as file to attach", data=full_body, file_name=f"{subject[:40].replace(' ','_')}.txt", key=f"email_dl_{key_suffix}")
        if len(content) > max_body:
            st.info("💡 Content was long, so the email body is truncated. Use 'Save as file to attach' to get the full version and attach it to your email.")
    ...
    if t1:
     with t1:
        ...
        if st.button("🌶️ Generate",type="primary",use_container_width=True,key="gen"):
            ...
            with dl_col:
                st.download_button("📥 Download",data=result,file_name=f"{task}_{topic}.txt".replace(" ","_")[:60],key="gen_dl")
            with em_col:
                pass  # placeholder for alignment
            email_result(result, f"Teacher Pehpeh — {task}: {topic} ({grade}, {subject})", "gen")
